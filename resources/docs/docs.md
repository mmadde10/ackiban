...
The `home-routes` handler in the `ackiban.routes.home` namespace
...
The `home-page` function will in turn call the `ackiban.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `ackiban.handler` namespace:
...
Request middleware functions are located under the `ackiban.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `ackiban.dev-middleware` namespace found in
